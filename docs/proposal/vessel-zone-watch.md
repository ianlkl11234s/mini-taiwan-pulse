...
- **回補**：分批 UPDATE，625k 筆一次跑完；只算中國類的話僅 ~207k 筆，但建議**全算**
  （台灣海巡署、他國執法船的相對位置日後同樣有敘事價值，且成本差不多）。
  ⚠️ **本節原估的耗時已被實測推翻，正確數字與修正方案見 §12.8a。**
...
## 12.8a ⚠️ 效能估算修正（2026-08-20 實測推翻本文原估）

**本文 §4.2 原寫「回補約 6 分鐘」「trigger 單筆約 2.4ms」——兩者都錯了約 70 倍。**

- 實測：**42 ms/筆**，全表 627,306 筆推算約 **7.3 小時**
- 錯誤來源：原估用 twmain 單一 region 的 6,503 點外推，但函數對**每一筆點要算全部 4 個 region**，
  而東沙／釣魚台的 24 浬線各有 **14,177 / 14,137 個頂點**（是 twmain 的 2.2 倍）
...
判斷依據：**14,177 個頂點對「距離幾浬」是過度精度**。帶寬是 6 浬、12 浬級距，0.06 浬誤差無意義。

**刻意不做 bbox 粗篩**：`dist_24nm_nm` 必須每一列都有值 ——
整個設計靠「zone 日後可從已存距離一次 UPDATE 重算」這個性質，
某些列沒有距離就毀了它。實測也顯示 99.97% 的點落在 200 浬 bbox 內，粗篩幾乎不具剔除效果。

**回補刻意不改寫成 set-based SQL**：那會變成兩份手工維護的五帶規則，日後必然悄悄分歧。
分類函數是唯一真相來源，回補批次呼叫它。
...
# 16. VZ-2 完成：回補與對帳結果（2026-08-20）

**覆蓋率 627,686 / 627,686（100%，零剩餘）**。8 月批次 107,581 筆耗時 131 秒（1.22 ms/筆）。
`VACUUM ANALYZE` 已跑。生產環境每小時 sweep 穩定 2.7 秒（全解析度時期是 19.4 秒）。

## 16.1 對帳表 1：三類 × 五帶（全期・四 region）

| 分類 | zone | 定位點 | 艘數 | 涉及天數 | POC 基準（本島） | 判讀 |
|---|---|---|---|---|---|---|
| 中國海事局 | contiguous | 39 | 3 | 1 | 39 / 3 | ✅ **完全吻合** |
| 中國海事局 | approach_6 | 109 | 3 | 2 | 108 / 3 | ✅ +1 |
| 中國海事局 | approach_12 | 139 | 3 | 3 | 131 / 3 | ✅ +8 |
| 中國海警 | contiguous | 1 | 1 | 1 | 1 / 1 | ✅ **完全吻合** |
| 中國海警 | approach_6 | 971 | 20 | 41 | 790 / 13 | ✅ +181 / +7 艘 |
| 中國海警 | approach_12 | 3,018 | 26 | 79 | 2,404 / 24 | ✅ +614 / +2 艘 |
| 中國科研船 | contiguous | 63 | 6 | 9 | 62 / 5 | ✅ +1 |
| 中國科研船 | approach_6 | 390 | 7 | 30 | 339 / 6 | ✅ +51 |
| 中國科研船 | approach_12 | 1,042 | 9 | 39 | 234 / 9 | ✅ +808 |

**判讀結果全部通過**：
- `territorial` 全部為 0（三批假 MMSI 已排除，POC 那個「9 天進領海」的假象確認消失）
- 九格**全部只增不減**，符合「加上東沙／釣魚台／黃岩島只會增加」的預期
- 兩格與 POC **完全吻合**（海事局 contiguous 39/3＝6/7 HAI XUN 三船編隊；海警 contiguous 1/1＝7/8 CCG 1401）
  → 兩套獨立幾何（POC 的 33/44 點簡化 vs 正式的 segmentize 365 點）算出同一個數字，判準鏈驗證成立

## 16.2 對帳表 2：按 region 拆分 —— **證實了「預設只算臺灣本島」的必要性**
...
**釣魚台合計 1,632 筆**，約當臺灣本島（4,131 筆）的四成 —— §3.3 的假設（混進主標題會淹沒台灣訊號）
**經實測成立**，Monitor 卡片預設只算臺灣本島、釣魚台／東沙／黃岩島做可切換附加列的設計維持不變。

⚠️ 一個值得後續追的異常：**釣魚台的 848 筆中國科研船來自單一一艘船**（1 艘 / 848 筆），
相當於長時間駐留而非過境。這是 POC 只算本島時完全看不到的現象，值得列入 VZ-7 事件化的首批案例。
