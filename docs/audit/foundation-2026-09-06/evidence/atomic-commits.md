...
上游 commit 的 Git 回滾只移除 SQL 檔案，**不代表還原已套用的資料庫**。初始提交時 migration 407 尚未套用；2026-09-06 已避開統計編號碰撞改為 408 並正式套用（見 member408-production.json）。上線後，先停會員寫入／回退前端，保留私人表與使用者資料，不能因回滾程式而 DROP TABLE。
