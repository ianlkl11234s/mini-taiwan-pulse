...
**目前進度（續作至批次 2）**：批次 0、1A、1B 的程式與本地驗收完成；批次 2 的會員保存 UI、loader/store、上游 migration 407 與隔離 DB 整合驗收完成。正式 Supabase 套用被自動核准審查擋下，尚待使用者對具體 migration 確認；已依使用者確認分組 commit；未 push、merge 或部署前端。
...
截至本次續作，1A／1B 已完成本地交付，2 的程式與隔離 DB 驗收完成。**下一步先完成 migration 407 的正式套用確認與實際 Google 帳號驗收，再安排前端發佈；批次 3 起維持後續計畫，本次未擴做。**
