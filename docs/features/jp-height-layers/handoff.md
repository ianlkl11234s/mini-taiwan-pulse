> **Terra 執行指南：[terra-runbook.md](terra-runbook.md)**。下列 2026-09-19 快照是較新的實際狀態；未再啟動全國大量下載。
...
>
> 驗證：installer pytest 3 passed；frontend focused 5 files / 22 tests passed；`npm run build` passed；本地 HTTP catalog 200 JSON，PMTiles Range 206；browser 如上通過。這是本地 CDN-ready 資產與 browser 證據，仍未 commit/push、上傳 runtime CDN、部署、啟用排程或完成 production 驗收。
...
- 永久 worktree：本 repo 與 `taipei-gis-analytics` 各自 `.worktrees/jp-height-layers-20260918`；branch `codex/jp-height-layers-20260918`。修改未提交，勿 reset/clean/stash。本輪僅有上段 S3 原始檔冷封存；沒有 commit、push、公開/CDN 上傳或部署。
