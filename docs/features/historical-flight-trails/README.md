...
前端預設從同源 `/flight-trails` 讀取，可用 `VITE_FLIGHT_TRAILS_CDN_BASE` 覆寫；沒有 DB 或 FR24 fallback。`20260919-v2` 已隨 PR [#316](https://github.com/ianlkl11234s/mini-taiwan-pulse/pull/316) 的一般 merge commit `9b6e9d88` 部署，Zeabur deployment `6aae9e2f` 為 `RUNNING`；正式 manifest 與桃園三日期、羽田 02/18 均通過 HTTP bytes、SHA-256、MIME 與 cache readback。Production desktop browser 已驗證全部機場預設 02/20、三日期載入、3D 藍白航跡與 console 無 error／warning；證據見 [`evidence/20260919-v2-production-deployment.json`](evidence/20260919-v2-production-deployment.json)、[`evidence/20260919-v2-production-http-readback.json`](evidence/20260919-v2-production-http-readback.json) 與 [`evidence/20260920-v2-production-browser.json`](evidence/20260920-v2-production-browser.json)。真機效能仍未完成，來源 `license_status=unverified`。
