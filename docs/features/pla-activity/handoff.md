...
> **2026-08-02 更新：本檔記錄的待辦已全部完成，保留作為決策軌跡。**
> 現況請看 [`README.md`](./README.md)（feature 全貌）與 [`changelog.md`](./changelog.md)（時序）；
> 後續工作看 [`backlog.md`](./backlog.md)。
>
> 三件當時的「待辦」後來的結果：
> | 當時狀態 | 現在 |
> |---|---|
> | ⚠️ 線上 collector 是舊版，每 30 分鐘覆蓋修好的資料 | ✅ 已部署（PR #41），資料自行修正、不需回填 |
> | 向量化 2026 通過率 69.9%，未達上線水準 | ✅ **85.4%**（項次分流 + 氣球圖徽抑制 + 已知目標數引導重試） |
> | 前端圖層規劃完成未動工 | ✅ A~D 期全上，四項待拍板已決（情勢 Situation / 先上 2026 / 待審進表加旗標 / 災害示警不搬） |
>
> 四個 repo 的未 push commit 也都已進 master/main。
