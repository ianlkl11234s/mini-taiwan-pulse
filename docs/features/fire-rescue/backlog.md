...
> memory 時點 2026-05-24 / 2026-05-26。當下三大待辦：commit/push / S3 部署 / 等時圈觀察。

## 進行中

- [ ] **FR-commit-push**：`feat/fire-rescue` 分支 headed browser 驗收通過但**尚未 commit / push**
- [ ] **FR-s3**：等時圈 PMTiles + 分隊/消防栓資產尚未 S3 部署
...
- fireEvents 為何「僅歷史模式」而 fireLatest「任何模式可見」— memory 有記結論但未記完整理由
- 等時圈是否已進 sidebar SECTIONS FIRE & RESCUE 區（memory 只說是 factory，未提 sidebar catalog 是否加）
