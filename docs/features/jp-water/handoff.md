...
Historical release gate (superseded on 2026-09-19): the eight PMTiles-backed national research layers were omitted from the production catalog and masked by the host. The owner-only private publication contract below replaces that UI mask: the catalog now shows locked entries to every visitor, while only the fixed owner account can obtain archive bytes through the authenticated Range API. The four previously released GeoJSON layers and the external official flood background remain public.
...
## 2026-09-19 owner-only private publication contract

The eight national vector layers no longer depend on a public CDN or a DEV localhost URL. They use two exact same-origin endpoints only:

- `/api/private-research/jp-water/water`
- `/api/private-research/jp-water/extra-water`

The production objects live in the dedicated `migu-private-research-ap-southeast-2` bucket with all four S3 Public Access Block controls enabled, BucketOwnerEnforced ownership, AES256 server-side encryption, and versioning. Keys are immutable and private: `private-research/jp-water/<sha256>/<filename>`. They are not included in `public/`, `/data`, the public deploy-assets sync, URL share state, embed, snapshot, or replay allowlists. Nginx proxies only the two exact archive names to the loopback sidecar and has no static/SPA fallback.

Every access probe, HEAD, and Range request carries the current Supabase bearer token. The sidecar calls Supabase `getUser`, accepts only the fixed owner account, requires a JWT `session_id`, checks the persistent revocation denylist, caps one Range at 8 MiB, and returns only HTTP 206 after exact SHA-256/bytes/ETag verification. Responses use `Cache-Control: private, no-store`; unauthorized/unknown paths fail closed. Logout revokes the shared private-research session before clearing local auth. A 401/403 removes the Mapbox source, all eight private layers, and any selected private feature.

The UI tier/lock is only a usability boundary; the sidecar is the security boundary. A fixed owner account does not make credential sharing technically impossible, so the operating rule remains: do not share the owner Google account or bearer token. This design substantially limits third-party access but does not itself resolve source-licence interpretation.

Release evidence stays separated: code/tests, private bucket upload, authenticated Range readback, anonymous/other-account denial, deployment, and production browser acceptance are distinct gates. Do not claim the private data is live until all gates have receipts.

Local owner acceptance at `2026-09-19`: the authenticated owner unlocked the eight private catalog entries and rendered nationwide supply and sewer points through the same-origin Range API. The supply legend showed seven conservative categories, and a popup identified `野尻浄水場` as a name-pattern-derived treatment plant with its 2010/P21/source-scoped identity. The sewer legend separated P22a/P22b; a popup identified `鴨川中ポンプ場` as a P22a pump station with its 2012/P22/source-scoped identity. Browser warning/error logs were empty. Logging out removed the private layer, URL selection, popup, and data immediately. This is local browser evidence, not production acceptance.

Private object acceptance at `2026-09-19`: both immutable objects were uploaded to the dedicated bucket. S3 HEAD returned the exact contract bytes, SHA-256 checksums, AES256 encryption, and matching `sha256` metadata. Independent full downloads reproduced both contract hashes; a credentialed `bytes=0-126` request returned exactly 127 bytes and the correct total, while both unsigned object URLs returned HTTP 403. Production sidecar readback remains a separate post-deployment gate.
