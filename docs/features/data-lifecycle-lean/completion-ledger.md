...
| 原計畫 | 已落地 | 證據與剩餘條件 |
|---|---|---|
| 1. 供應／快取 | nginx MIME/gzip/public cache，公開 R2 單層 | production-304.json、r2-pilot.json；Cloudflare zone eligibility 與帳單需管理權限 |
...
1. Cloudflare token 看不到 itsmigu.com zone；AWS帳單權限先前遭拒。需登入管理頁或提供已限制權限的現有 connector；請勿在聊天貼secret。
2. news_events/yt_live_history 尚無可驗證 archive＋保留決策；保持 HOLD。資料保留與費用有取捨，不能以未定義天數自動刪除。
