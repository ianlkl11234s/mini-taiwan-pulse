...
在 Monitor 以每日直條呈現公開目錄中中國 ISR-capable 候選衛星的星下點穿越臺灣本島 12 浬領海範圍次數，並在 tooltip 顯示當日不重複衛星數；可切換 30／90／120 個日曆日，比較最新日與期間中位數，並以相對色階標示所選期間內的過境量位階。
...
- 柱色依所選 30D／90D／120D 內可呈現日的 Type-7 `p25 / p50 / p75 / p90` 重算：綠／灰／黃／橙／紅依序代表基準低段、基準中段、相對偏多、相對高量、相對高峰；跨期間的顏色不可直接互比。
- 至少 8 個可呈現日才啟用相對分級；最新完整日嚴格高於 p90 才顯示相對量提醒，p75–p90 只顯示橙色位階。色階與提醒只描述公開軌道推算的相對過境量，不是安全、威脅、任務執行或實際蒐情分級。
