...
- 瀏覽器實測（z4.7 全日本）：渲染 17,069 條線、5 類齊全
  ...
  ⚠️ 17,069 是 z4.7 的**瀏覽器端渲染計數**，**不等於**資料總量 21,933。差額已查明非資料遺失：
  PMTiles `strategies` 各 zoom 皆空（無任何 drop／coalesce）、`tilestats.count` = 21,933 完整；
  少的是視窗外（沖繩／九州南緣）與低 zoom 併段。
