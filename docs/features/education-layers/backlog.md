...
- [ ] **EDU-14**：上游 `cram_schools` PMTiles 用 `--drop-densest-as-needed` 切片，
      **z8~z14 每層都大量丟點，只有 z15 完整**（實測台北同一 bbox：z12 只有 15 筆、
      z13 35、z14 84、z15 204）。前端已在圖例誠實標示抽稀。

      **2026-08-09 已實測重切代價**（跑過 `tippecanoe -o … -l cram_schools -Z8 -z15
      -r1 --no-feature-limit --no-tile-size-limit`，不必再跑一次）：

      | | 現行 | 重切 |
      |---|---:|---:|
      | 檔案大小 | 3,984,654 (3.80 MB) | **11,147,037 (10.63 MB)** |
      | 各 zoom 丟點 | z8 丟 17,284 … z14 丟 10,511 | **完全不丟**（`strategies` = null）|
      | 教育主題資產總量 | 16.55 MB | 23.3 MB |

      **代價 2.8 倍，收益不確定** —— 17,137 點在 z10-12 的台北市區全畫出來很可能
      糊成一片色塊，比現在更難讀。且重切要改的是**上游 pipeline 參數**，屬上游決定。
      2026-08-09 owner 拍板：**維持現況**（抽稀 + 圖例標示），不重切。
      若日後改變主意，上面的數字直接可用。
