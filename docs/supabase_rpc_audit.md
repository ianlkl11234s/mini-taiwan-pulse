...
| 21 | `get_global_event_places_current` | globalEventsLoader.ts:267 | `intel.global_events` + `intel.global_event_versions` + `intel.global_event_places` | `p_category, p_before_published_at, p_limit`（≤100，預設50） | 無（JOIN + `LIMIT`，非聚合） | 有（`published_at DESC, event_id, place_key`，但先用 CTE `LIMIT` 夾住候選事件再 join place） | ❌ 無 | 未量測 | ② 現況 OK（新上線） | 🟡 未量測 | `STABLE SECURITY DEFINER`，`GRANT` 給 `anon, authenticated`；上線初期先觀察，量大再跑 `/check-rpc` |
| 22 | `get_global_event_places_window` | globalEventsLoader.ts:286 | `intel.global_events` + `intel.global_event_versions` + `intel.global_event_places` + `intel.global_event_publisher_places` | `p_window_start, p_window_end, p_category, p_limit_events`（≤100，預設100） | 無（half-open display 區間 join，非聚合） | 未量測 | ❌ 無 | 未量測 | ② 現況 OK（新上線） | 🟡 未量測 | `STABLE SECURITY DEFINER`，`GRANT` 給 `anon, authenticated`；跨月大 window 建議先跑 `/check-rpc` 驗證 plan |
| 23 | `get_global_event_candidates_window` | globalEventsLoader.ts:196 | `intel.global_event_candidate_observations` + `intel.global_event_version_candidate_refs` + `intel.global_event_versions` | `p_window_start, p_window_end, p_limit_candidates`（≤1000，前端每頁請求 200）, `p_after_candidate_id` | `jsonb_agg` 組 JSONB envelope（`rows/total_candidates/has_more/next_after_candidate_id`），`LATERAL jsonb_array_elements` 展開 places | 無全表排序；`page` CTE 用 `ORDER BY candidate_id LIMIT` 做 keyset 分頁 | ❌ 無 | 未量測（每頁 ≤200 candidates，前端迴圈翻頁到 `has_more=false`） | ② 現況 OK（新上線） | 🟡 未量測 | `STABLE SECURITY DEFINER`，`GRANT` 給 `anon, authenticated`；函式本身限制 window 跨度 ≤31 天；前端已用 `cachedByKey`（5 分鐘 / 8 entries）；量大留意 `(observed_at)`／`(candidate_id)` index |
| 24 | `ingest_global_event_candidates` | 不在本 repo（service_role only，由收集端寫入） | `intel.global_event_candidate_observations`（write） | `p_candidates jsonb`（≤100 筆/次） | 無（單次 append-only INSERT，canonical-content 重放不建版本） | 無 | ❌ 無 | 未量測 | ⑤ 寫入（非讀路徑） | — | `LANGUAGE plpgsql VOLATILE SECURITY DEFINER`，`GRANT` 僅 `service_role`（`anon/authenticated` 皆 `REVOKE`），前端不可呼叫；驗證見 gis-platform `scripts/test_global_event_candidates.sql` |
