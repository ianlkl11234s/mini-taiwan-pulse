 ...
 * 所以本支改成由應用端從 `OVERLAY_REGISTRY` 算出「首載後應常駐的 sourceId」，
 * 再跟瀏覽器裡實際存在的對帳 —— 精確、不需基準線、而且直接指名是哪幾層沒建起來。
 * 依 viewport 動態管理的 source（目前為 JP height）不屬於常駐集合，
 * 應由各自 lifecycle 的 browser acceptance 驗證，不可把 template id 當成缺失。
 *
 * ── 刻意不做的事 ──────────────────────────────────────────────
 *
