...
class HistoricalFlightTrailExportTests(unittest.TestCase):
    def record(self, **changes):
        base = {"fr24_id": "a", "origin_icao": "RCTP", "dest_icao": "RJTT", "dest_icao_actual": "RJTT", "dep_time": 1773093600, "arr_time": 1773104400, "path": [[25, 121, 10, 100], [26, 122, 20, 200]]}
        base.update(changes)
        return base
    ...
    def test_tw_and_jst_event_days_and_actual_destination(self):
        record = self.record(
            dep_time=int(datetime(2026, 3, 10, 0, 30, tzinfo=ZoneInfo("Asia/Taipei")).timestamp()),
            arr_time=int(datetime(2026, 3, 10, 1, 30, tzinfo=ZoneInfo("Asia/Tokyo")).timestamp()),
            dest_icao="ZZZZ", dest_icao_actual="RJTT",
        )
        self.assertEqual(flight_export.event_roles(record, "RCTP", "2026-03-10", "Asia/Taipei"), ["departure"])
        self.assertEqual(flight_export.event_roles(record, "RJTT", "2026-03-10", "Asia/Tokyo"), ["arrival"])

    def test_missing_times_do_not_claim_roles(self):
        record = self.record(dep_time=0, arr_time=0)
        self.assertEqual(flight_export.event_roles(record, "RCTP", "2026-03-10", "Asia/Taipei"), [])
    ...
    def test_feature_preserves_valid_points_and_unknown_route_scope(self):
        feature = flight_export.feature_for_record(self.record(dest_icao="ZZZZ", dest_icao_actual="RJTT", path=[[1, 1, 4, 1], [2, 2, 5, 2]]), "RCTP", "2026-03-10", "Asia/Taipei", {}, {"RJTT": "HND"})
    ...
    def test_export_writes_utf8_hashed_immutable_asset_and_unavailable_samples(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            departure = int(datetime(2026, 3, 10, 8, tzinfo=ZoneInfo("Asia/Taipei")).timestamp())
            (source_root / "RCTP" / "2026-03-10.jsonl").write_text(json.dumps(self.record(dep_time=departure)), encoding="utf-8")
            ...
            sample = next(item for item in manifest["samples"] if item["airport"] == "RCTP" and item["date"] == "2026-03-10")
            ...
            self.assertEqual(next(item for item in manifest["samples"] if item["airport"] == "RCKW" and item["date"] == "2026-03-10")["coverage"], "unavailable")
    ...
    def test_existing_source_without_drawable_feature_is_unavailable_without_asset(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            (source_root / "RCTP").mkdir(parents=True)
            departure = int(datetime(2026, 3, 10, 8, tzinfo=ZoneInfo("Asia/Taipei")).timestamp())
            (source_root / "RCTP" / "2026-03-10.jsonl").write_text(json.dumps(self.record(dep_time=departure, path=[[25, 121, 0, 1]])), encoding="utf-8")
            airport_points = root / "airports.json"
            airport_points.write_text(json.dumps(self.airport_points_payload()), encoding="utf-8")
            output = root / "public" / "flight-trails"
            manifest = flight_export.export(source_root, output, "r1", airport_points, "2026-09-18T00:00:00Z")
