...
import {
  ...
  type LatestWinsGate,
  type LatestWinsQueueSnapshot,
  type ManualVisualGate,
...
/** Burst/continuous probe 都不 await producer tick；逐 tick await 會掩蓋 queue。 */
const BURST_TICKS = 16;
const CONTINUOUS_TICKS = 24;
const CONTINUOUS_POST_INTERVAL_MS = 4;
const REPLY_TIMEOUT_MS = 60_000;
const rootUrl = () => new URLSearchParams(location.search).get("root") ?? GFW_V4_SPATIAL_ROOT_URL;
type Samples = { updates: number; p95Ms: number; maxMs: number };
type FrameReply = GfwV4TrackFrameReply;
type QueueProbe = { telemetry: LatestWinsQueueSnapshot; gate: LatestWinsGate };
type Metrics = {
  schemaVersion: 2;
  ...
  playbackQueue: { burst: QueueProbe; continuousPost: QueueProbe };
  grid: {
    gate: "not-run";
    desiredHour: null;
    renderedHour: null;
  ...
  nativeGpuVisual: ManualVisualGate;
...
const isFrameReply = (reply: GfwV4TrackWorkerReply): reply is GfwV4TrackFrameReply => reply.ok && !("type" in reply);
...
function App() {
  ...
  /** Queue probe 保證同時最多一筆 ask；generation + frameEpoch 用來核對回覆。 */
  ...
  const run = async () => {
    ...
    try {
      ...
      const loadedReply = await ask(target, { type: "load", generation, epoch, trailingSeconds, tiles, assets: assets.map((asset) => ({ url: resolveGfwV4SpatialArtifactUrl(asset, rootUrl()), bucket: GFW_V4_TRACK_BUCKETS.indexOf(asset.bucket), identity: `${asset.bucket}|${asset.observedAt}` })) });
      if (!isFrameReply(loadedReply)) { setStatus(!loadedReply.ok ? `worker error: ${loadedReply.error}` : "unexpected pick reply during load probe"); return; }
      const loaded = loadedReply;
      ...
      const sendRender = async (requestedEpoch: number): Promise<FrameReply> => {
        const mark = performance.now();
        const tick = await ask(target, { type: "render", generation, epoch: requestedEpoch, trailingSeconds, includeHits: false });
        renderSamples.push(performance.now() - mark);
        if (!isFrameReply(tick)) throw new Error(!tick.ok ? `worker error: ${tick.error}` : "unexpected pick reply during render probe");
        if (!tick.loaded || tick.generation !== generation || tick.frameEpoch !== requestedEpoch) {
          throw new Error(`render reply mismatch: requested=${requestedEpoch}, replied=${tick.frameEpoch}, generation=${tick.generation}/${generation}, loaded=${tick.loaded}`);
        }
        return tick;
      };
      const applyFrame = (tick: FrameReply) => {
        // This is the same typed-buffer upload input as production. It is not a
        // proxy for Mapbox CustomLayer pixels or click picking.
        scene.updateSpatialPoints({ points: tick.points, buckets: tick.buckets, memberCounts: tick.memberCounts, segments: tick.segments, segmentBuckets: tick.segmentBuckets }, profiles[profile].zoom);
      };

      // Synchronous producer burst: all epochs are posted without awaiting a
      // reply. The first reply must be dropped and only the final pending epoch
      // may be applied.
      const burstQueue = new BoundedLatestWinsQueue(sendRender, applyFrame);
      const burstEpochs = Array.from({ length: BURST_TICKS }, (_, index) => epoch + 60 + index * 15);
      burstEpochs.forEach((tickEpoch) => burstQueue.enqueue(tickEpoch));
      const burstTelemetry = await burstQueue.whenIdle();
      const burstGate = evaluateLatestWinsGate(burstTelemetry, burstEpochs[burstEpochs.length - 1]!, { requireCoalescing: true });

      // Continuous producer: posts keep arriving while Worker replies may be in
      // flight. There is deliberately no per-tick await in this loop.
      const continuousQueue = new BoundedLatestWinsQueue(sendRender, applyFrame);
      const continuousEpochs = Array.from({ length: CONTINUOUS_TICKS }, (_, index) => epoch + 900 + index * 20);
      for (const tickEpoch of continuousEpochs) {
        continuousQueue.enqueue(tickEpoch);
        await new Promise<void>((resolve) => setTimeout(resolve, CONTINUOUS_POST_INTERVAL_MS));
      }
      const continuousTelemetry = await continuousQueue.whenIdle();
      const continuousGate = evaluateLatestWinsGate(continuousTelemetry, continuousEpochs[continuousEpochs.length - 1]!);
      scene.dispose();
      ...
      setMetrics({
        ...
        tileCache, frameWork: summarize(frameSamples), renderTick: summarize(renderSamples),
        playbackQueue: {
          burst: { telemetry: burstTelemetry, gate: burstGate },
          continuousPost: { telemetry: continuousTelemetry, gate: continuousGate },
        },
        grid: {
          gate: "not-run",
          desiredHour: null,
          renderedHour: null,
          holdingDurationMs: null,
          reason: "Phase-2 spatial bench has no Mapbox Grid sources; use GridHoldingRecorder in the main-map probe.",
        },
        nativeGpuVisual: unresolvedNativeGpuGate("This page uploads typed buffers without a Mapbox canvas; run the main-map native-GPU head/trail/click probe."),
        heap,
