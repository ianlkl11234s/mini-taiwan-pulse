...
function setup(execute = vi.fn().mockResolvedValue({ camera: [121, 25] }), onActivity = vi.fn()) {
  ...
  const responder = new QueryResponder({ client, studyId: "study", tabId: "tab" } as unknown as BridgeConnectionContext, execute, vi.fn(), onActivity);
  return { client, responder, execute, onActivity };
}
