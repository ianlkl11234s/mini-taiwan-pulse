...
export type BrowserQuery = { requestId: string; operation: "search_layers" | "describe_layer" | "read_layer" | "map_context" | "find_places" | "nearby" | "search_datasets" | "describe_dataset" | "query_records" | "spatial_query" | "aggregate_records" | "join_records" | "calculate_metric" | "get_data_quality" | "get_record_evidence" | "get_analysis_result" | "get_result_bounds" | "list_results" | "remove_result"; args: Record<string, unknown>; expiresAt: number };
...
function isQueryEnvelope(value: unknown): value is { request: BrowserQuery | null } {
  ...
  return request === null || exactObject(request, ["requestId", "operation", "args", "expiresAt"]) && safeId(request.requestId) && typeof request.operation === "string" && ["search_layers", "describe_layer", "read_layer", "map_context", "find_places", "nearby", "search_datasets", "describe_dataset", "query_records", "spatial_query", "aggregate_records", "join_records", "calculate_metric", "get_data_quality", "get_record_evidence", "get_analysis_result", "get_result_bounds", "list_results", "remove_result"].includes(request.operation) && isObject(request.args) && typeof request.expiresAt === "number" && Number.isFinite(request.expiresAt);
}
