...
export function useTransportParams() {
  ...
  // ── 雲林 POC 覆蓋分析 5 layer params ──
  const [gasCoverageAllOpacity, setGasCoverageAllOpacity] = useState(0.55);
  const [gasCoverageCpcOpacity, setGasCoverageCpcOpacity] = useState(0.25);
  const [gasCoverageFpccOpacity, setGasCoverageFpccOpacity] = useState(0.25);
  const [gasCoverageTaisugarOpacity, setGasCoverageTaisugarOpacity] = useState(0.25);
  ...
  const overlayParams = useMemo<Record<string, number>>(() => ({
    ...
    // 雲林 POC 覆蓋分析
    gasCoverageAllOpacity,
    gasCoverageCpcOpacity,
    gasCoverageFpccOpacity,
    gasCoverageTaisugarOpacity,
    evIslandOpacity,
