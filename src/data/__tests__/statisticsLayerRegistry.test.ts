...
describe("statistics layer role registry", () => {
  ...
  it("只接四份 manifest 白名單的 15 個交通統計 release，並保留層級與語意", () => {
    expect(taipeiBicycleReleaseSelector.resolve({ release_id: "2021-110-column7-0f6dc7a55e33", period_start: "2021-01-01", period_end: "2021-12-31" })).toEqual({ releaseId: "2021-110-column7-0f6dc7a55e33", dimensions: { roc_year: "110", source_field: "COLUMN7", geographic_coverage: "taipei_township_only" } });
    ...
    expect(caaAirportActivityReleaseSelector.resolve({ release_id: "2026-07-caa-airport-airport_cargo_tonnes-21a7e7f4-ff82aa92fc47", period_start: "2026-07-01", period_end: "2026-07-31" })).toEqual({ releaseId: "2026-07-caa-airport-airport_cargo_tonnes-21a7e7f4-ff82aa92fc47", dimensions: { roc_year: "115", month: "07", geographic_semantics: "facility_location_activity", health: "CURRENT", coverage: "PARTIAL" } });
    ...
    expect(taoyuanAirportReleaseSelector.resolve({ release_id: "2022-taoyuan-airport-airport_passenger_movements-d6d399a1-9aa1f1a56d6b", period_start: "2022-01-01", period_end: "2022-12-31" })).toEqual({ releaseId: "2022-taoyuan-airport-airport_passenger_movements-d6d399a1-9aa1f1a56d6b", dimensions: { year: "2022", airport_iata: "TPE", airport_icao: "RCTP", geographic_semantics: "facility_location_activity", health: "STALE", refresh: "manual" } });
