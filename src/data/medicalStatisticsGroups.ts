...
import { COMPARISON_ENABLED_RECIPES } from './comparisonStatisticsRecipes';
...
const comparisonOptions = (indicator: string) => COMPARISON_ENABLED_RECIPES
...
export const MEDICAL_STATISTICS_GROUPS = [
  ...
  { key: "hospitalBeds", label: "醫院病床", optionLabel: '指標', options: [option("statsHealthHospitalBedTotal", "全部：原始數"), option("statsHealthAcuteBedTotal", "急性：原始數"), option("statsHealthIcuBedTotal", "加護：原始數"), option("statsHealthHospiceBedTotal", "安寧：原始數"), ...COMPARISON_ENABLED_RECIPES.filter(r => /(?:hospital|acute|icu|hospice)_bed_total_per_(?:10000_population|km2)_township/.test(r.indicator_id)).map(r => option(r.layer_key, r.optionLabel))] },
  { key: "hospitalWorkforce", label: "醫院人力", optionLabel: '指標', options: [option("statsHealthHealthProfessionalTotal", "醫事人員總計：原始數"), option("statsHealthWesternPhysicianCount", "西醫師：原始數"), option("statsHealthRegisteredNurseCount", "護理師：原始數"), ...COMPARISON_ENABLED_RECIPES.filter(r => /(?:health_professional_total|western_physician_count|registered_nurse_count)_per_(?:10000_population|km2)_township/.test(r.indicator_id)).map(r => option(r.layer_key, r.optionLabel))] },
  { key: 'nursingBeds65plus', label: '一般護理之家床位', optionLabel: '口徑', options: [option('statsHealthGeneralNursingHomeOpenBeds','開放床數'), ...COMPARISON_ENABLED_RECIPES.filter(r => r.indicator_id==='general_nursing_home_open_beds_per_65plus_1000_county').map(r=>option(r.layer_key, `${r.level === 'county' ? '縣市' : '鄉鎮'}：${r.optionLabel}`))] },
  { key: 'careWorker65plus', label: '照顧服務員登錄', optionLabel: '口徑', options: [option('statsHealthCareWorkerRegistration','登錄數'), ...COMPARISON_ENABLED_RECIPES.filter(r => r.indicator_id==='care_worker_registration_count_per_65plus_1000_county').map(r=>option(r.layer_key, `${r.level === 'county' ? '縣市' : '鄉鎮'}：${r.optionLabel}`))] },
  { key: 'postpartumBedsBirths', label: '產後護理床', optionLabel: '口徑', options: [option('statsHealthPostpartumNursingHomeOpenBeds','開放護理床數'), ...COMPARISON_ENABLED_RECIPES.filter(r => r.indicator_id==='postpartum_nursing_home_open_beds_per_births_1000_county').map(r=>option(r.layer_key, `${r.level === 'county' ? '縣市' : '鄉鎮'}：${r.optionLabel}`))] },
  { key: 'postpartumInfantBedsBirths', label: '產後護理嬰兒床', optionLabel: '口徑', options: [option('statsHealthPostpartumNursingHomeOpenInfantBeds','開放嬰兒床數'), ...COMPARISON_ENABLED_RECIPES.filter(r => r.indicator_id==='postpartum_nursing_home_open_infant_beds_per_births_1000_county').map(r=>option(r.layer_key, `${r.level === 'county' ? '縣市' : '鄉鎮'}：${r.optionLabel}`))] },
...
export const HOUSING_STATISTICS_GROUPS: StatisticsToggleGroup[] = HOUSING.map(([id,label,county,township]) => ({ key:`housing${id}`,label:`住宅：${label}`,optionLabel:'地理層級／口徑',options:[option(county,'縣市：原始數'),option(township,'鄉鎮：原始數'),...(id === 'occupied' ? [option('statsHousingOccupiedPctCounty','縣市：占總住宅比'),option('statsHousingOccupiedPctTownship','鄉鎮：占總住宅比')] : id === 'unused' ? [option('statsHousingUnusedPctCounty','縣市：占總住宅比'),option('statsHousingUnusedPctTownship','鄉鎮：占總住宅比')] : []),...COMPARISON_ENABLED_RECIPES.filter(r=>r.indicator_id===`housing_${id}_share_pct_county`||r.indicator_id===`housing_${id}_share_pct_township`).map(r=>option(r.layer_key, `${r.level === 'county' ? '縣市' : '鄉鎮'}：${r.optionLabel}`))]}));
export const HOUSING_MIXED_STATISTICS_GROUP: StatisticsToggleGroup = { key:'housingMixedUse',label:'有人居住住宅用途',optionLabel:'指標',options:[option('statsHousingResidenceOnlyCounty','住宅專用（縣市原始數）'),option('statsHousingMixedUseCounty','兼農工服務業用（縣市原始數）'),...COMPARISON_ENABLED_RECIPES.filter(r=>r.indicator_id==='housing_mixed_share_of_residential_or_mixed_pct_county').map(r=>option(r.layer_key, `${r.level === 'county' ? '縣市' : '鄉鎮'}：${r.optionLabel}`))] };
...
export const LAND_STATISTICS_GROUPS: StatisticsToggleGroup[] = LAND.map(([stem,label,raw])=>({key:`land${stem}`,label:`土地：${label}用地`,optionLabel:'地理層級／比較口徑',options:[option(raw,'鄉鎮：原始面積'),...COMPARISON_ENABLED_RECIPES.filter(r=>r.layer_key.startsWith(`statsComparison${stem}`)).map(r=>option(r.layer_key, `${r.level === 'county' ? '縣市' : '鄉鎮'}：${r.optionLabel}`))]}));
