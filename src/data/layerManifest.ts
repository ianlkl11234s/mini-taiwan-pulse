...
/**
 * Catalog aliases are bridge-only: recipe.dataset_id stays the runtime release
 * contract, while the catalog keeps the canonical family id.
 */
export const AGRI_CATALOG_DATASET_ALIASES: Readonly<Record<string, string>> = {
  fishery_stats_by_county: "fishery_stats",
  moa_crop_township_statistics_7302: "crop_township_statistics",
};
 ...
 */
const COMPARISON_UPSTREAM: UpstreamRef = {
  status: 'catalog_missing',
  datasets: [],
  processing: 'Pulse offline derived Statistics family; exact runtime release allowlist and regionalStatisticsMap processing remain local.',
  note: 'Catalog entry pending; this bridge status does not change the runtime comparison_statistics recipe contract.',
};
...
export const LAYER_MANIFEST = {
  ...
  jpHistoricalFlightTrails: {
    ...
    upstream: {
      status: "catalog_missing",
      datasets: [],
      processing: "歷史航班樣本；依機場、日期載入已保留的實測航跡。",
      note: "Coverage 以 manifest 為準；catalog entry pending，不等於完整航班或即時軌跡。",
    },
    dataClass: "D",
    source: {
      kind: "custom",
  ...
  tainanBridgeInspections: {
    ...
    dataClass: "B",
    source: { kind: "pmtiles", sourceId: "tainan-bridge-inspections", url: "./network_structures/tainan_bridge_inspections_20260924.pmtiles", sourceLayer: "tainan_bridge_inspections", minzoom: 7, maxzoom: 15 },
    legend: "tainanBridgeInspections",
    popup: "tainanBridgeInspections",
    params: { count: 2, kinds: ["slider", "slider"] },
    description: "臺南 112–113 年 1,719 筆檢測紀錄點；名稱含天橋、地下道，不等於實體道路橋數或即時安全狀態。",
    topics: ["交通", "橋梁", "檢測", "臺南"],
  },
  officialBridgesHsinchu: {
    ...
    legend: "officialBridgesHsinchu", popup: "officialBridgesHsinchu",
    params: { count: 2, kinds: ["slider", "slider"] },
    description: "新竹市官方清冊 175 筆；有名稱、路線及登錄長度，端點連線不是橋身幾何。",
    topics: ["交通", "橋梁", "新竹"],
  },
  taipeiRoadTunnels: {
    ...
    description: "臺北 13 個官方登錄點，含 21 筆方向明細；不是洞口或隧道路廊。",
    topics: ["交通", "隧道", "臺北"],
  },
  tainanRoadTunnels: {
    ...
    description: "臺南 6 個官方登錄點；來源未宣告 CRS，非隧道線形或通行狀態。",
    topics: ["交通", "隧道", "臺南"],
  },
  changhuaTrafficSignals: {
    ...
    description: "彰化縣 3,448 個可定位號誌清冊點；非獨立路口數，也沒有即時燈態。",
    topics: ["交通", "號誌", "路口", "彰化"],
  },
  bridgeRainThresholds: {
    ...
    dataClass: "D",
    source: { kind: "custom", note: "useBridgeRainLayer 從 Supabase get_rain_gauge_latest 讀取即時雨量，依前期記錄的公路局 1150904 門檻計算雨量條件；現行來源版本待覆核，非封橋或安全狀態。" },
    legend: "bridgeRainThresholds",
    popup: "bridgeRainThresholds",
    params: { count: 1, kinds: ["slider"] },
    description: "10 座公路局一級監控橋梁的參考雨量試點；表版次待覆核，缺測與未覆核條件顯示未知，位置為 OSM 同名候選。",
    topics: ["交通", "橋梁", "雨量", "監控"],
  },
  jpPoliceFacilities: {
