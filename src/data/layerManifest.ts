...
export const LAYER_MANIFEST = {
  ...
  ooklaMobilePerformance: {
    ...
    upstream: {
      status: "catalog_missing",
      datasets: [],
      note: "Ookla Speedtest 使用者樣本聚合格網；非 coverage map；CC BY-NC-SA 4.0 ⚠️ 上游尚未在 taipei-gis-analytics catalog 建檔，故 status 為 catalog_missing 而非 verified。原宣告 dataset id：`network_performance_grid`（catalog 規範為全小寫底線且須等於檔名，帶點的 id 需一併改名）。建檔後改回 verified 並填回 datasets。",
    },
    dataClass: "A",
