...
export const LAYER_MANIFEST = {
  ...
  urbanZoningTaipei: {
    ...
    params: { count: 2, kinds: ["multiSelect", "slider"] },
  ...
  culturalFacilities: {
    ...
    params: { count: 3, kinds: ["multiSelect", "slider", "slider"] },
  ...
  gfwHourlyGrid: {
    ...
    popup: "gfwHourlyGrid",
    params: { count: 1, kinds: ["slider"] },
    description: "0.1° GFW 小時 vessel presence 格網；色階與透明度依船舶數連續插值，點擊列出格內完整船舶",
    topics: ["世界", "海事", "船舶", "GFW", "時間軸", "網格"],
  },
  ...
  gfwHourlyTracks: {
    ...
    upstream: {
      status: "catalog_missing",
      datasets: [],
      processing: "schema-4 selected-day PMTiles frames；固定 z6 Range Worker 載入 H-1/H/H+1，並以 content-addressed detail shards 補全 popup",
      note: "正式 root/release 均需 bytes/SHA-256、taxonomy 與 release truth 驗證；每個頂點是 GFW 格網近似位置，非原始 AIS 精確航跡",
    },
    dataClass: "D",
    source: {
      kind: "custom",
      note: "優先同域 /global-maritime/gfw-hourly/v4/manifest.json；驗證過的 schema-4 release 以 PMTiles Range Worker 載入，v2/v3 僅在 formal root 不可用時 fallback",
      staticAssets: ["./gfw_hourly_tracks_poc/manifest.json"],
    },
    legend: "gfwHourlyTracks",
    popup: "gfwHourlyTrack",
    params: { count: 8, kinds: ["slider", "select", "toggle", "toggle", "toggle", "toggle", "toggle", "toggle"] },
    description: "GFW schema-4 小時船舶近似航跡；跟隨時間軸載入 H-1/H/H+1，六船種獨立選取、同座標完整聚合",
  ...
  gfwFishingEffort: {
    ...
    upstream: {
      ...
      note: "schema-4 daily fishing effort 與 Grid、Tracks、SAR 各自獨立；apparent fishing effort 不是漁獲量或違法捕撈判定",
    },
    dataClass: "D",
    source: {
      kind: "custom",
      note: "優先同域 /global-maritime/gfw-hourly/v4/manifest.json；驗證 root/release 與 immutable gzip artifact 後載入，舊版僅作 fallback",
      staticAssets: [],
    },
  ...
  gfwDarkVessels: {
    ...
    upstream: {
      status: "catalog_missing",
      datasets: [],
      processing: "GFW SAR presence matched=false；依 UTC 小時切分未與 AIS 匹配的偵測格點",
      note: "2026-08-25 live schema probe 已驗證 public-global-sar-presence:v4.0 wrapper；空時段仍會產生 0-feature asset；不可詮釋為違法或確認關閉 AIS",
    },
    dataClass: "D",
    source: {
      kind: "custom",
  ...
  jpWaterDams: { key: "jpWaterDams", section: { theme: "水資源", group: "水資源靜態資料" }, label: "水壩 ダム（2014）", expandable: true, color: "#0369a1", icon: Dam, upstream: { status: "verified", datasets: [{ datasetId: "jp_water_w01_dams", confidence: "HIGH" }] }, dataClass: "D", source: { kind: "custom", note: "DEV-only local water.pmtiles; source-layer=dams; SHA/bytes preflight", staticAssets: ["LOCAL_ONLY: water.pmtiles"] }, legend: "jpWaterDams", popup: "jpWaterDams", params: { count: 1, kinds: ["slider"] }, description: "2014 歷史水壩點位；非即時蓄水量或全國現況。", topics: ["日本", "水資源", "水壩", "歷史", "LOCAL_ONLY"] },
  ...
  jpWaterRivers: { key: "jpWaterRivers", section: { theme: "水資源", group: "水資源靜態資料" }, label: "河川流路 河川（2006–2009）", expandable: true, color: "#0284c7", icon: Route, upstream: { status: "verified", datasets: [{ datasetId: "jp_water_w05_rivers", confidence: "HIGH" }] }, dataClass: "D", source: { kind: "custom", note: "DEV-only local water.pmtiles; source-layer=rivers; segments are not river count", staticAssets: ["LOCAL_ONLY: water.pmtiles"] }, legend: "jpWaterRivers", popup: "jpWaterRivers", params: { count: 1, kinds: ["slider"] }, description: "2006–2009 歷史河川流路線段；不是河川條數，端點已排除。", topics: ["日本", "水資源", "河川", "歷史", "LOCAL_ONLY"] },
  jpWaterSupplyFacilities: { key: "jpWaterSupplyFacilities", section: { theme: "水資源", group: "水資源靜態資料" }, label: "上水道相關設施（2010）", expandable: true, color: "#06b6d4", icon: Droplets, upstream: { status: "verified", datasets: [{ datasetId: "jp_water_p21_supply", confidence: "HIGH" }] }, dataClass: "D", source: { kind: "custom", note: "DEV-only local water.pmtiles; source-layer=supply; subtypes are conservative name-pattern classifications", staticAssets: ["LOCAL_ONLY: water.pmtiles"] }, legend: "jpWaterSupplyFacilities", popup: "jpWaterSupplyFacilities", params: { count: 1, kinds: ["slider"] }, description: "2010 上水道相關設施；依設施名稱保守分類，未命中者保留未分類。", topics: ["日本", "水資源", "上水道", "歷史", "LOCAL_ONLY"] },
  jpWaterSupplyAreas: { key: "jpWaterSupplyAreas", section: { theme: "水資源", group: "水資源靜態資料" }, label: "給水區域 給水区域（2010）", expandable: true, color: "#7dd3fc", icon: Waves, upstream: { status: "verified", datasets: [{ datasetId: "jp_water_p21_supply_areas", confidence: "HIGH" }] }, dataClass: "D", source: { kind: "custom", note: "DEV-only local water.pmtiles; source-layer=supply_areas", staticAssets: ["LOCAL_ONLY: water.pmtiles"] }, legend: "jpWaterSupplyAreas", popup: "jpWaterSupplyAreas", params: { count: 1, kinds: ["slider"] }, description: "2010 給水區域歷史面；不是事業者數，低透明度以保留其他點選。", topics: ["日本", "水資源", "上水道", "歷史", "LOCAL_ONLY"] },
  jpWaterSewerFacilities: { key: "jpWaterSewerFacilities", section: { theme: "水資源", group: "水資源靜態資料" }, label: "下水道設施（2012）", expandable: true, color: "#1d4ed8", icon: Container, upstream: { status: "verified", datasets: [{ datasetId: "jp_water_p22_sewer", confidence: "HIGH" }] }, dataClass: "D", source: { kind: "custom", note: "DEV-only local water.pmtiles; source-layer=sewer; subtype from P22a/P22b", staticAssets: ["LOCAL_ONLY: water.pmtiles"] }, legend: "jpWaterSewerFacilities", popup: "jpWaterSewerFacilities", params: { count: 1, kinds: ["slider"] }, description: "2012 下水道設施點；依 P22a／P22b 區分泵場與處理場，不代表完整地下管線。", topics: ["日本", "水資源", "下水道", "歷史", "LOCAL_ONLY"] },
  ...
  jpWaterGroundwaterSites: { key: "jpWaterGroundwaterSites", section: { theme: "水資源", group: "補充資料（覆蓋與定位限制見圖例）" }, label: "地下水等觀測點（24縣）", expandable: true, color: "#8b5cf6", icon: MapPin, upstream: { status: "verified", datasets: [{ datasetId: "jp_water_gsj_groundwater", confidence: "HIGH" }] }, dataClass: "D", source: { kind: "custom", note: "DEV-only local extra-water.pmtiles; source-layer=groundwater", staticAssets: ["LOCAL_ONLY: extra-water.pmtiles"] }, legend: "jpWaterGroundwaterSites", popup: "jpWaterGroundwaterSites", params: { count: 1, kinds: ["slider"] }, description: "GSJ 地下水・湧水・河川地点，實際僅24縣，年份未提供。", topics: ["日本", "水資源", "地下水", "LOCAL_ONLY"] },
  jpWaterNilimDams: { key: "jpWaterNilimDams", section: { theme: "水資源", group: "補充資料（覆蓋與定位限制見圖例）" }, label: "NILIM 水壩位置（46縣）", expandable: true, color: "#2563eb", icon: Dam, upstream: { status: "verified", datasets: [{ datasetId: "jp_water_nilim_dams", confidence: "HIGH" }] }, dataClass: "D", source: { kind: "custom", note: "DEV-only local extra-water.pmtiles; source-layer=nilim", staticAssets: ["LOCAL_ONLY: extra-water.pmtiles"] }, legend: "jpWaterNilimDams", popup: "jpWaterNilimDams", params: { count: 1, kinds: ["slider"] }, description: "NILIM 水壩位置，實際46縣、年份未提供；不可與 KSJ 水壩相加。", topics: ["日本", "水資源", "水壩", "LOCAL_ONLY"] },
  jpWaterAgriculturalPonds: { key: "jpWaterAgriculturalPonds", section: { theme: "水資源", group: "補充資料（覆蓋與定位限制見圖例）" }, label: "農業蓄水池（2026-03）", expandable: true, color: "#65a30d", icon: Droplets, upstream: { status: "verified", datasets: [{ datasetId: "jp_water_maff_agricultural_ponds", confidence: "HIGH" }] }, dataClass: "D", source: { kind: "custom", note: "DEV-only local extra-water.pmtiles; source-layer=agri; source datum unknown", staticAssets: ["LOCAL_ONLY: extra-water.pmtiles"] }, legend: "jpWaterAgriculturalPonds", popup: "jpWaterAgriculturalPonds", params: { count: 1, kinds: ["slider"] }, description: "MAFF 2026-03 農業蓄水池位置候選；來源 datum 未知，保留4,284同座標列。", topics: ["日本", "水資源", "農業", "LOCAL_ONLY"] },
  jpWaterFloodHazard: { key: "jpWaterFloodHazard", section: { theme: "水資源", group: "背景" }, label: "洪水浸水想定（最大規模）", expandable: true, color: "#64748b", icon: CloudRain, upstream: { status: "verified", datasets: [{ datasetId: "gsi_flood_l2_shinsuishin", confidence: "HIGH" }] }, dataClass: "D", source: { kind: "custom", note: "Official GSI XYZ raster z2–17; disabled by default; source/terms: disaportal.gsi.go.jp hazard-map open-data pages; no feature attributes/popup", staticAssets: ["https://disaportaldata.gsi.go.jp/raster/01_flood_l2_shinsuishin_data/{z}/{x}/{y}.png"] }, legend: "jpWaterFloodHazard", popup: null, params: { count: 1, kinds: ["slider"] }, description: "官方最大規模洪水浸水想定背景；空白不等於無風險，非即時災情或預報。", topics: ["日本", "水資源", "洪水", "背景"] },
  ...
  ooklaMobilePerformance: {
    ...
    upstream: {
      status: "verified",
      datasets: [{ datasetId: "network_performance_grid", confidence: "HIGH" }],
      note: "Ookla Speedtest 使用者樣本聚合格網；非 coverage map；CC BY-NC-SA 4.0。2026-Q1 官方 z16 Parquet 聚合，產物為 --slim（整層常數在 FeatureCollection metadata／前端 OOKLA_GRID_META，不逐格重複）。",
    },
    dataClass: "A",
    source: { kind: "geojson", sourceId: "ookla-mobile-global", url: "./geo/ookla_mobile_global.geojson" },
  ...
  ooklaFixedPerformance: {
    ...
    dataClass: "A",
    source: { kind: "geojson", sourceId: "ookla-fixed-global", url: "./geo/ookla_fixed_global.geojson" },
    legend: "ooklaPerformanceGrid",
  ...
  ooklaMobileTaiwan: {
    ...
    dataClass: "B",
    source: [
      { kind: "pmtiles", sourceId: "ookla-tw-z14-mobile", url: "./geo/ookla_tw_z14.pmtiles", sourceLayer: "mobile", minzoom: 6, maxzoom: 14 },
      { kind: "pmtiles", sourceId: "ookla-tw-z16-mobile", url: "./geo/ookla_tw_z16.pmtiles", sourceLayer: "mobile", minzoom: 15, maxzoom: 16 },
    ],
    legend: "ooklaPerformanceGrid",
    popup: "ooklaMobileGrid",
    params: { count: 2, kinds: ["select", "slider"] },
    description: "台灣範圍行動網路細格：z14（約 2.4 km）拉近自動換 z16 原生格（610 m）；使用者樣本，不代表覆蓋範圍",
    topics: ["通訊", "網路效能", "Ookla", "台灣"],
  },
  ...
  ooklaFixedTaiwan: {
    ...
    dataClass: "B",
    source: [
      { kind: "pmtiles", sourceId: "ookla-tw-z14-fixed", url: "./geo/ookla_tw_z14.pmtiles", sourceLayer: "fixed", minzoom: 6, maxzoom: 14 },
      { kind: "pmtiles", sourceId: "ookla-tw-z16-fixed", url: "./geo/ookla_tw_z16.pmtiles", sourceLayer: "fixed", minzoom: 15, maxzoom: 16 },
    ],
    legend: "ooklaPerformanceGrid",
    popup: "ooklaFixedGrid",
    params: { count: 2, kinds: ["select", "slider"] },
    description: "台灣範圍固定網路細格：z14（約 2.4 km）拉近自動換 z16 原生格（610 m）；WiFi／乙太連線的測速樣本",
    topics: ["通訊", "網路效能", "Ookla", "台灣"],
  },
  ...
  streetTreesNational: {
    ...
    params: { count: 4, kinds: ["select", "multiSelect", "slider", "slider"] },
