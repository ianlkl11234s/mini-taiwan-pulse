...
export const LAYER_MANIFEST = {
  ...
  globalEvents: {
    ...
    upstream: {
      ...
      note: "Live 讀 anon-safe current RPC；Replay 讀 immutable window RPC。事件原始位置仍是 SSOT；城市／國家代表中心須有 publisher proxy lineage，unknown geometry 不顯示",
    },
    dataClass: "D",
    source: {
      kind: "custom",
      note: "useGlobalEventsLayer 自建 global-events-current GeoJSON source/circle；Live 取 current、Replay 取最多 100 個事件的 immutable window，本地依 display interval 切片並保留同版本多地點",
    },
    legend: "globalEvents",
    popup: "globalEvent",
    params: { count: 1, kinds: ["slider"] },
    description: "已發布的全球重要事件回放；顏色表示分類、大小表示嚴重度，可畫來源支持的事件點與有 lineage 的城市／國家代表中心",
    topics: ["世界", "重要事件", "情報"],
  },
