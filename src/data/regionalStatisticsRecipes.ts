...
const TAIPEI_BICYCLE_DIMENSIONS: Record<string, Record<string, string>> = {
  '2021-110-column1-4de5e4875a10': { roc_year: '110', source_field: 'COLUMN1', geographic_coverage: 'taipei_township_only' },
  '2021-110-column3-f9ec2899a8ff': { roc_year: '110', source_field: 'COLUMN3', geographic_coverage: 'taipei_township_only' },
  '2021-110-column5-cdef44619364': { roc_year: '110', source_field: 'COLUMN5', geographic_coverage: 'taipei_township_only' },
  '2021-110-column6-cae6b58fd0b3': { roc_year: '110', source_field: 'COLUMN6', geographic_coverage: 'taipei_township_only' },
  '2021-110-column7-0f6dc7a55e33': { roc_year: '110', source_field: 'COLUMN7', geographic_coverage: 'taipei_township_only' },
};
...
const CAA_AIRPORT_RELEASES = new Set(['2026-07-caa-airport-takeoffs_landings_count-21a7e7f4-caf184008ff5', '2026-07-caa-airport-airport_passenger_movements-21a7e7f4-5ad7bd5e6d97', '2026-07-caa-airport-airport_cargo_tonnes-21a7e7f4-ff82aa92fc47']);
...
const TAOYUAN_AIRPORT_RELEASES = new Set(['2022-taoyuan-airport-airport_arrivals_count-d6d399a1-ad3c8de6bd45', '2022-taoyuan-airport-airport_departures_count-d6d399a1-2952dfe52009', '2022-taoyuan-airport-airport_transit_count-d6d399a1-b888d90ebf8b', '2022-taoyuan-airport-airport_passenger_movements-d6d399a1-9aa1f1a56d6b']);
...
export const STATISTICS_RECIPES = {
  ...
  statsAirportTakeoffsLandings: { dataset_id: 'caa_airport_activity_county_33238', indicator_id: 'takeoffs_landings_count', level: 'county', label: '機場所在地起降架次', unit: '次', frequency: '每月（115 年 7 月）', releaseId: '2026-07-caa-airport-takeoffs_landings_count-21a7e7f4-caf184008ff5', dimensions: CAA_AIRPORT_DIMENSIONS, includeHealth: true, releaseSelector: caaAirportActivityReleaseSelector, interpretationNote: '依機場所在地彙總，非居民或航空公司所在地；16 個數值機場中 15 個落界，望安 16 架次未分配，恆春連字號保留缺值。', breaks: [100, 500, 2_000, 10_000], colors: ['#eff6ff', '#bfdbfe', '#60a5fa', '#2563eb', '#1e3a8a'] },
  statsAirportPassengerMovements: { dataset_id: 'caa_airport_activity_county_33238', indicator_id: 'airport_passenger_movements', level: 'county', label: '機場所在地旅客人次', unit: '人次', frequency: '每月（115 年 7 月）', releaseId: '2026-07-caa-airport-airport_passenger_movements-21a7e7f4-5ad7bd5e6d97', dimensions: CAA_AIRPORT_DIMENSIONS, includeHealth: true, releaseSelector: caaAirportActivityReleaseSelector, interpretationNote: '依機場所在地彙總，含入境、出境與過境，不是居民或不重複旅客；望安 196 人次未分配，恆春連字號保留缺值。', breaks: [10_000, 100_000, 500_000, 2_000_000], colors: ['#f0fdf4', '#bbf7d0', '#4ade80', '#16a34a', '#14532d'] },
  statsAirportCargoTonnes: { dataset_id: 'caa_airport_activity_county_33238', indicator_id: 'airport_cargo_tonnes', level: 'county', label: '機場所在地貨運量', unit: '公噸', frequency: '每月（115 年 7 月）', releaseId: '2026-07-caa-airport-airport_cargo_tonnes-21a7e7f4-ff82aa92fc47', dimensions: CAA_AIRPORT_DIMENSIONS, includeHealth: true, releaseSelector: caaAirportActivityReleaseSelector, interpretationNote: '依機場所在地彙總，不是居民或航空公司所在地；望安 0.017 公噸未分配，恆春連字號保留缺值。', breaks: [100, 1_000, 10_000, 100_000], colors: ['#f5f3ff', '#ddd6fe', '#a78bfa', '#7c3aed', '#4c1d95'] },
  statsTaoyuanAirportArrivals: { dataset_id: 'taoyuan_airport_passengers_county_32997', indicator_id: 'airport_arrivals_count', level: 'county', label: '桃園機場所在地入境旅客人次', unit: '人次', frequency: '2022 年歷史快照', freshness: 'STALE', releaseId: '2022-taoyuan-airport-airport_arrivals_count-d6d399a1-ad3c8de6bd45', dimensions: TAOYUAN_AIRPORT_DIMENSIONS, includeHealth: true, releaseSelector: taoyuanAirportReleaseSelector, interpretationNote: '僅桃園市 TPE/RCTP 設施所在地有觀測值，其他 21 縣市為缺資料；不是 0、居民或不重複旅客。', breaks: [100_000, 500_000, 1_000_000, 2_000_000], colors: ['#ecfeff', '#a5f3fc', '#22d3ee', '#0891b2', '#164e63'] },
  statsTaoyuanAirportDepartures: { dataset_id: 'taoyuan_airport_passengers_county_32997', indicator_id: 'airport_departures_count', level: 'county', label: '桃園機場所在地出境旅客人次', unit: '人次', frequency: '2022 年歷史快照', freshness: 'STALE', releaseId: '2022-taoyuan-airport-airport_departures_count-d6d399a1-2952dfe52009', dimensions: TAOYUAN_AIRPORT_DIMENSIONS, includeHealth: true, releaseSelector: taoyuanAirportReleaseSelector, interpretationNote: '僅桃園市 TPE/RCTP 設施所在地有觀測值，其他 21 縣市為缺資料；不是 0、居民或不重複旅客。', breaks: [100_000, 500_000, 1_000_000, 2_000_000], colors: ['#eff6ff', '#bfdbfe', '#60a5fa', '#2563eb', '#1e3a8a'] },
  statsTaoyuanAirportTransit: { dataset_id: 'taoyuan_airport_passengers_county_32997', indicator_id: 'airport_transit_count', level: 'county', label: '桃園機場所在地過境旅客人次', unit: '人次', frequency: '2022 年歷史快照', freshness: 'STALE', releaseId: '2022-taoyuan-airport-airport_transit_count-d6d399a1-b888d90ebf8b', dimensions: TAOYUAN_AIRPORT_DIMENSIONS, includeHealth: true, releaseSelector: taoyuanAirportReleaseSelector, interpretationNote: '僅桃園市 TPE/RCTP 設施所在地有觀測值，其他 21 縣市為缺資料；不是 0、居民或不重複旅客。', breaks: [1_000, 5_000, 10_000, 20_000], colors: ['#fff7ed', '#fed7aa', '#fb923c', '#ea580c', '#7c2d12'] },
  statsTaoyuanAirportPassengerMovements: { dataset_id: 'taoyuan_airport_passengers_county_32997', indicator_id: 'airport_passenger_movements', level: 'county', label: '桃園機場所在地旅客人次總計', unit: '人次', frequency: '2022 年歷史快照', freshness: 'STALE', releaseId: '2022-taoyuan-airport-airport_passenger_movements-d6d399a1-9aa1f1a56d6b', dimensions: TAOYUAN_AIRPORT_DIMENSIONS, includeHealth: true, releaseSelector: taoyuanAirportReleaseSelector, interpretationNote: '僅桃園市 TPE/RCTP 設施所在地有觀測值，其他 21 縣市為缺資料；這是總計，禁止再與入境、出境、過境指標相加。', breaks: [100_000, 1_000_000, 3_000_000, 5_000_000], colors: ['#f5f3ff', '#ddd6fe', '#a78bfa', '#7c3aed', '#4c1d95'] },
