...
import { X, Waves, DoorOpen, Droplets } from "lucide-react";
...
function TaipeiSewerPanel({ props }: { props: Record<string, unknown> }) {
...
function TaipeiPumbPanel({ props }: { props: Record<string, unknown> }) {
...
function TaipeiEvacuatePanel({ props }: { props: Record<string, unknown> }) {
  const stationNo = String(props.station_no ?? "");
  const name = String(props.station_name ?? stationNo);
...
function Badge({ label, on, color }: { label: string; on: boolean; color: string }) {
  return (
    <span style={{
      fontSize: 10,
      padding: "1px 5px",
...
function MedicalPOIPanel({ props }: { props: Record<string, unknown> }) {
...
function HikingTrailsPanel({ props }: { props: Record<string, unknown> }) {
...
export function FeatureInfoPanel({ feature, onClose, reservoirContext }: Props) {
  ...
  if (isReservoir && hasCompareId && reservoirContext?.reservoir) {
    ...
    case "floodSensorIsochrone":
      ...
      break;
    case "taipeiSewer":
      content = <TaipeiSewerPanel props={feature.properties} />;
      break;
    case "taipeiEvacuate":
      content = <TaipeiEvacuatePanel props={feature.properties} />;
      break;
    case "taipeiPumb":
      content = <TaipeiPumbPanel props={feature.properties} />;
      break;
    case "wasteFacility":
    ...
    case "forestryPOI":
      ...
      break;
    case "hikingTrails":
      content = <HikingTrailsPanel props={feature.properties} />;
      break;
    case "agriPOI":
