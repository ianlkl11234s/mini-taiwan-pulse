...
- **雙軸共振**：擾台嚴重度的分級方式。規模軸＝架次百分位、強度軸＝逾越中線百分位，
  `base = max(兩軸)` 分五級；**兩軸皆 ≥p90 時再升一級**。存在理由是 `max()` 分不出
  「(p92,p92) 協同大規模」與「(p95,p10) 單一維度衝高」。門檻取 90 是實測選的
  （75/85 會讓「顯著」佔到 1/8 天）
- **嚴重度五級**：平靜 / 一般 / 偏高 / 高 / 顯著，對應百分位 <50 / <75 / <90 / <97 / ≥97。
  近 120 天分布 43/40/22/7/8
- **氣球圖徽**：航跡圖上空飄氣球的畫法 —— 實心編號圈＋箭頭＋**虛線圈**，不是活動區。
  判別式：小 cluster 閉合後只有 ~7×9px 的圓孔（編號圈內部）而**沒有大孔**
- **days_ago**：`get_pla_tracks_range()` 回傳欄，＝視窗結束日 − 該形狀日期。
  0＝最新、`trailDays-1`＝最舊。前端用它做新舊淡化，以及**累積回放**
  （filter `days_ago >= thresh`，thresh 由大遞減 → 形狀由舊到新逐日長出來）
- **混合機型項次**：航跡圖表格的 `(3 sorties of fighter / UAV)` 是這兩種**合計** 3 架次，
  拆不出各自幾架。399 項次中單一機型僅 205（51%）→ 機型的可信主指標是「出現天數」而非架次

## 殯葬 Funeral（2026-08-05 加）

- **A／B／C 三源分開**（殯葬）：2026-08-05 拍板的原則 —— 官方名冊點（A・OGDL）、
  OSM 墓區面（B・**ODbL 必須標示**）、都計墓葬類法定用地（C・OGDL，**僅臺北＋新北**）
  三個來源**各自獨立 toggle，前端不合併不去重**。同一座公墓在三源各出現一次是
  **刻意保留的重複** —— 使用者同時開 B 與 C 看到的落差（實際使用 vs 法定劃設）本身就是資訊。
  對照組：aquaculture 的 `union_class`（both/moa_only/osm_only）走的是相反策略（先合併再標來源）
- **precision 標記族**（殯葬點位）：座標是四段 fallback 拼出來的，欄位值標記該點怎麼來的。
  `source`（官方原生）／`exact`（門牌級）／`cached`（TGOS 快取）／`tgos`（官方比對）／
  `interpolated`（同路段內插）／**`parcel_centroid`（地籍 bbox 中心，誤差 1.4–126 m）**／
  **`approximate`（鄉鎮／路段中心，可能差數百公尺）**。後兩者佔設施 **42%** ——
  做「最近的火化場」這類距離分析**必須先濾掉**，純密度展示則無妨。
  前端做法：popup 誠實標「位置為概略值」＋ 提供三態 filter，不默默全畫也不默默全濾
- **遷他縣市**（商工登記 status）：業者遷址後，**舊縣市的舊登記仍留在名冊**。
  2026-08-06 前 `is_active` 誤判為 true → 同一統編同時出現在新舊兩地，圖上看起來像兩家在營業。
  修正後歸入「已失效」。⚠️ 這也是為什麼**登記類資料的 PK 不能只用實體識別碼**
  （統編＋名稱會把遷址前後壓成一筆），要含地址或時點

## 落雷雙源 / ledger（2026-08-07 加）

| 詞 | 意思 |
|---|---|
| **ledger 表** | pipeline 的執行帳本，**每個處理過的單位必有一列**（不論產出幾筆）。存在理由：產出表在「0 筆是合法結果」時分不出「沒事發生」與「沒跑」。例：`spatial.pla_tracks_runs`（mig 337） |
| **`source` 欄位（落雷）** | `taipower` = 台電 d546005；`cwa` = 氣象署 O-A0039-001。同一張 `live.lightning_events`，UNIQUE 改成 `(source, event_id)` / `(source, dedup_hash)` 兩源各自去重（mig 338） |
| **snapshot 整檔覆寫** | 台電落雷的供應方式：每分鐘整個檔案被下一批覆蓋，**錯過 60 秒就永久遺失** → 這種源不能降頻 |
| **滾動視窗** | 氣象署落雷的供應方式：每次回傳「最近 1 小時」，每 5 分更新。cron 慢幾分鐘不會漏，代價是同一筆被重複抓約 12 次（靠 dedup_hash 擋） |
| **`RECOVERY_GAP_DAYS`** | 判定「上游斷供後恢復」的門檻（3 天）。判準是「DB 上一筆距今多久」而非「這輪有沒有資料」—— 後者每場雷雨都會誤報 |
| **假成功配準（off-by-N）** | `solve_georef()` 回傳了看似合理的解，但網格線抓錯 N 格，整幅圖平移最多 3 度。2026-05~07 實測佔 16%，`needs_review` 抓不到（只驗形狀數不驗位置） |
| **`needs_review`（共機）** | 守門 = 「抽出形狀數 == 圖上表格期望數」。不一致者仍入表但標記，兩支 RPC 預設排除。⚠️ **只驗數量不驗位置** |

## Embed 動態回放（2026-08-09 加，EM-16）

| 詞 | 意思 |
|---|---|
| **保存層（`trails/`）** | S3 上「為了不讓 retention 吃掉而每晚匯出」的原始日檔，**不是給前端讀的**。不在 `deploy-assets/` 下、不經 nginx／Cloudflare。要拿來畫圖必須先加工成成品包再放進供檔路徑 |
| **成品包** | 從保存層加工出來、體積與欄位都為「畫這一張圖」裁切過的檔案（例：`embed-snapshots/<layer>/<date>.json.gz`）。保存層求完整、成品包求小 —— 兩者刻意分離 |
| **回放快照（replay snapshot）** | 凍結某一天的動態圖層資料，讓嵌入頁在**零 Supabase 請求**下播完整天。檔名含日期 → 可 1 年 immutable 快取 |
| **軌跡插值型** | flights／ships 的回放型別：快照存的是**逐點軌跡**，引擎在兩點之間插值。資料量隨船／機數線性長 |
...
| **LayerHost** | `src/layers/` 的掛載元件：每個 layer hook 包成 `return null` 的小元件，由 `layerHookRegistry.tsx` 的有序 registry（74 entry）在 App 內固定迴圈渲染。順序＝原 App.tsx 呼叫順序凍結（effect/疊放語意）。每個 Host 自己 `useLayerParams(key)` per-key 訂閱 → 拖 slider 只 re-render 該 Host |
| **三桶（registry／HOOKS_IN_APP／NO_HOOK）** | `layerHookRegistry.test.ts` 對全 348 manifest key 的互斥分類斷言：有 Host 的（133）／回傳值被 App 消費留在 App 的（37）／純 overlay 無 hook 的（178）。漏掛 hook 會紅=第 5 個靜默失敗點的機械護欄。⚠️ 已知盲區：7 個雙桶 key 不設防（W3 待補） |
