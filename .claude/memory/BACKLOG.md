...
| PI-1 | P2 | 警察 isochrone 5 區邊界斷裂修正 | open | 2026-07-01 全台跑後發現：`policeIsoSubstation/Precinct/CityDept` 3 層 isochrone 在 5 區（north/north2/central/south/east）交界處有明顯斷裂 — 邊界 station 的 `ego_graph` 只在本區 graph 上跑，跨區被截斷，polygon 在區界斷開（見圖：桃園/新竹交界、雲林/嘉義交界最明顯）。修法有 3 種：**A** 5 區 bbox 各 +0.15° (~16km) overlap，station 在 overlap 帶雙區都跑後 dedup（重跑 ~60-90 min）；**B** 改用 raster heatmap（每 station burn 到 grid 後累加 overlap_count，完全避開 vector polygon 邊界問題，工程 1-2 天）；**C** 分區跑後做「補丁 pass」— 對區界 buffer 內的邊界 station 用「跨區 graph」(全台或該 station 半徑內全區) 重跑一次覆蓋。**同時要處理**：line layer 造成的「同心圓邊界感」— 2026-07-01 前端已把 `line-opacity 0.3→0.08` `line-width 0.3→0.15` 淡化，若 raster 路線就不需要 line layer 了。pipeline 位置：`taipei-gis-analytics/pipelines/police_justice/isochrone/`（15_run_by_region.sh + 16_merge_regions.py + 20_merge_combined.py + 10_police_isochrone.py 主 script） |
