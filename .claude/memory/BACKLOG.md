...
| AR-21~26 | P2 | 圖層架構：細粒度 visibility/params store + Layer Manifest | **主體 done 2026-08-12（PR #130）** | AR-21 #129＋AR-22/23/24 #130：348 層登記＋336 參數全遷 manifest/spec、useTransportParams 退役（→useLayerParamsRuntime 570 行）、layerConsistency 改守 manifest 完整性（5 靜默點解 4.5）。**未竟 8 條 SSOT → [`docs/features/layer-manifest/backlog.md`](../../docs/features/layer-manifest/backlog.md)**（AR-22 終點 per-key 訂閱兌現／4b legend-popup 派生化／App.tsx hook 迴圈＝最後半個靜默點等）。⑤④ 已於 08-12 先行結案（06_apply 改對帳器 d1f9479／fireHydrants 改名同步 b6f0e55） |
