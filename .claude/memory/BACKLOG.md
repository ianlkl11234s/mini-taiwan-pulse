...
| BL-24 | P1 | `get_waste_schedule_day` pre-aggregate（實測 48s）| open | `/check-rpc` 2026-07-22 實測 48.06s、來源 193k stops → 2,978 route groups，前端 30s 逾時**圖層載不出**（用戶回報的 console timeout）。每呼叫即時重算（逐列 regex + 每列 3× ST_Distance geography + 多輪 window + jsonb_agg）。結果只依 `p_dow`（7）×`p_cities`、來源靜態 → 物化成表（dow, city, route_id, …, stops jsonb）+ refresh function（資料更新時重跑，未必需每日 cron），RPC 改薄 `SELECT … WHERE dow=X`，可降到 <100ms。**DB migration（gis-platform），待用戶拍板**；用 supabase-optimize skill 產範本。診斷詳見 INCIDENTS 2026-07-22 事件 B |
