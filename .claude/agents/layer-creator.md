...
7. **Edit** `src/components/sidebar/layerCatalog.ts`（單一真實來源，桌機 IconRailSidebar 與手機 LayerSidebar 共用）：
   - ⚠️ **最重要**：`LAYER_COLORS` 加 `<layerKey>: "#XXXX"`（不加會 tsc error TS2739）
   - `SECTIONS` 對應分區加入該 layer key
   - UI toggle 的渲染仍在兩個 sidebar 元件，但資料來源已統一在此檔
